...
const getDashboardData = async (user, targetUserId = null) => {
  ...
  if (user.role === "ADMIN" && targetUserId) {
    ...
    if (targetUser.role === "MEMBER")
      return await dashboardModel.getMemberStats(targetUserId);
    if (targetUser.role === "PM")
      return await dashboardModel.getPMStats(targetUserId);
  ...
  switch (user.role) {
    case "ADMIN":
      return await dashboardModel.getAdminStats();
    case "PM":
      return await dashboardModel.getPMStats(user.id);
    case "MEMBER":
      return await dashboardModel.getMemberStats(user.id);
    default:
      return {};
  }
};
