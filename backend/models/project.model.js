    ...
    try {
        ...
        if (name || description || status) {
            ...
            if (fields.length > 0) {
                ...
                await client.query(`UPDATE projects SET ${fields.join(', ')} WHERE id = $${values.length}`, values);
            }
        }
        ...
        if (managerIds && Array.isArray(managerIds)) {
            // Reset quyền cũ
            await client.query(`UPDATE project_members SET is_manager = FALSE WHERE project_id = $1`, [projectId]);
            // Set quyền mới
